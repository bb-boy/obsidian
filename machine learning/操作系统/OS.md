...
**异常和不可屏蔽中断**的中断向量号是由 CPU 自动提供的，而来自外部设备的可屏蔽中断号是由中断代理提供的（咱们这里的中断代理是 8259A），软中断是由软件提供的
...
16 位的表界限，表示最大范围是 0xffff，即 64KB。可容纳的描述符个数是 64KB/8=8K=8192 个。第 0 个门描述符也是可用的，**中断向量号为 0 的中断是除法错**。但处理器只支持 256 个中断，即 0～255，中断描述符中其余的描述符不可用。在门描述符中有个 P 位，所以，咱们将来在构建 IDT 时，
- **P=1**：门有效，CPU 允许通过这扇门进入处理程序（会做权限检查、压栈、可能切栈等）。
- **P=0**：门无效/不存在。CPU 想用这项时，会**产生异常**（常见是 `#NP` 或 `#GP`，取决于具体触发路径），而不是跳去执行你填的 offset。
...
> [!PDF|note] [[操作系统真象还原 (郑纲).pdf#page=322&selection=150,43,166,22&color=note|操作系统真象还原 (郑纲), p.322]]
> > 在准备用 iret 指令返回时，当前**栈指针 esp 必须指向栈中备份的 EIP_old** 所在的位置，这样处理器才能将栈中的数据对号入座，弹出到各自对应的寄存器中。
...
|       |          |                                  |                 |                       |
![[machine learning/操作系统/shot/image 3.png]]
...
![[machine learning/操作系统/shot/image-1.png]]
...
![[machine learning/操作系统/shot/image-2.png]]
**从片的 ICW3（编号）**
...
![[machine learning/操作系统/shot/image-4.png]]
...
![[machine learning/操作系统/shot/image-5.png]]
...
|     D1 | **L1**  | 低位（二进制位）                                                                             |
|     D0 | **L0**  | 低位（二进制位）                                                                             |
![[machine learning/操作系统/shot/image-6.png]]
...
在 x86 PC 架构中，有一个硬件定时器芯片（Intel 8253 或 8254 PIT - Programmable Interval Timer）：
┌─────────────────────────────────────┐
│  8253/8254 定时器芯片               │
│  ─────────────────────────────────  │
│  • 连接到主板晶振（1.193182 MHz）   │
│  • 默认配置：每 1/18.2 秒触发一次   │
│  • 自动向 8259A PIC 的 IRQ0 发信号 │
└──────────────┬──────────────────────┘
               │ 硬件信号
               ↓
┌─────────────────────────────────────┐
...
**内部时钟**
内部时钟是指处理器中内部元件，如运算器、控制器的工作时序，主要用于控制、同步内部工作过程的步调。内部时钟是由晶体振荡器产生的，简称晶振，它位于主板上，其频率经过分频之后就是主板的外频，处理器和南北桥之间的通信就基于外频。
**主频**外频乘以某个倍数（也称为倍频）
...
![[image-9.png]]计数器 0～ 计数器 2，它们的端口分别是 0x40～0x42。计数器又称为通道，都是 16 位大小。它们是独立的，也就是这三个计数器的工作是不依赖的，可以同时工作。各个计数器都有自己的一套寄存器资源。**寄存器资源包括一个 16 位的计数初值寄存器、一个计数器执行部件和一个输出锁存器。**
> [!PDF|important] [[操作系统真象还原 (郑纲).pdf#page=360&selection=128,0,185,14&color=important|操作系统真象还原 (郑纲), p.360]]
> 每个计数器都有三个引脚：CLK，GATE，OUT。 （1）CLK 表示时钟输入信号，即计数器自己工作的节拍，也就是计数器自己的时钟频率。每当此引脚收到一个时钟信号，减法计数器就将计数值减 1。连接到此引脚的脉冲频率最高为 10MHz，8253 为 2MHz。 （2）GATE 表示门控输入信号，在某些工作方式下用于控制计数器是否可以开始计数，在不同工作方式下 GATE 的作用不同，到时候同工作方式一同介绍。 （3）OUT 表示计数器输出信号。当定时工作结束，也就是计数值为 0 时，根据计数器的工作方式， 会在 OUT 引脚上输出相应的信号。此信号用来通知处理器或某个设备：定时完成。这样处理器或外部设备便可以执行相应的行为动作。

**计数初值寄存器**：CPU 写入“起始值 N”的地方，只负责把初值准备好。开始计数后，它本身不参与递减；在某些模式（如 2、3）到 0 会自动把初值再装回去继续下一轮。

**计数器执行部件**：真正的“16 位递减计数器”。从初值寄存器装载 N 后按时钟一路减，直到终点触发输出；计数过程中初值寄存器的内容保持不变。

**输出锁存器**：为了解决“既要持续计数、又要读到一致的当前值”的矛盾。读数前先把“此刻的计数值”快照到锁存器，CPU 再从锁存器读，这样既不打断递减也避免读到变化中的值；锁存的快照不会自动更新，需再次锁存

计数器的工作就是以计数的形式实现定时功能，计数时间到达后就会发一个输出信号， 此信号可以向处理器发中断。因此，计数器 0 的计时到期后就会发出时钟中断信号，中断代理 8259A 就会感知引脚 IRQ0 有中断信号到来
...
> [!PDF|important] [[操作系统真象还原 (郑纲).pdf#page=365&selection=214,0,289,1&color=important|操作系统真象还原 (郑纲), p.365]]

...
16 位的表界限，表示最大范围是 0xffff，即 64KB。可容纳的描述符个数是 64KB/8=8K=8192 个。第 0 个门描述符也是可用的，**中断向量号为 0 的中断是除法错**。但处理器只支持 256 个中断，即 0～255，中断描述符中其余的描述符不可用。在门描述符中有个 P 位，所以，咱们将来在构建 IDT 时，记得把 P 位置 0，这样就表示门描述符中的中断处理程序不在内存中。

...
2. **16位结构拆解**: 一个选择子被精确地划分为三个部分：![[Pasted image 20250828195132.png]]
...
好的，根据您提供的五张图片内容，这里为您总结一下其中介绍的三种利用BIOS `int 0x15`中断来获取物理内存大小的方法。

这三种方法可以看作是BIOS内存检测功能的一个演进过程，从简单但有限，到复杂但全面。
...
> ([[操作系统真象还原 (郑纲).pdf#page=199&selection=96,0,111,28&color=important|操作系统真象还原 (郑纲), p.199]])
> 在保护模式下，段描述符是内存段的身份证。CPU 在引用一个段时，都要先查看段描述符。很多时候，段描述符存在于描述符表中（GDT 或 LDT），但与此对应的段并不在内存中，也就是说，CPU 允许在描述符表中已注册的段不在内存中存在，这就是它提供给软件使用的策略，
